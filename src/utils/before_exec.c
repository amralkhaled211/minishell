...
/*   Updated: 2024/03/26 14:59:08 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	remove_quotes(t_shell *shell, t_command *command)
{
	...
	{
		errno = 0;
		temp = handle_quotes(shell, command[i].cmd_name, 2);
...
void	rm_quotes_args(t_shell *shell, t_command *command)
{
	...
	{
		temp = handle_quotes(shell, command->args[j], 2);
		free(command->args[j]);
		command->args[j] = temp;
		j++;
	}
...
void	rm_quotes_redir(t_shell *shell, t_command *command)
{
	...
	while (command->redirections && command->redirections[j].file)
	{
		...
		temp = handle_quotes(shell, command->redirections[j].file, 2);
