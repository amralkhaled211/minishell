...
/*   Updated: 2024/03/19 17:46:57 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rm_quotes_args(t_shell *shell, t_command *command)
{
	...
	{
		if (command->expanded_args[j] && ft_strncmp("export",
			command->cmd_name, ft_strlen("export")))
		{
			j++;
			continue ;
		}
...
void	rm_quotes_redir(t_shell *shell, t_command *command)
{
	...
	while (command->redirections && command->redirections[j].file)
	{
		...
		if (command->redirections[j].expanded_redir)
		{
			j++;
			continue ;
		}
