...
/*   Updated: 2024/03/20 21:57:56 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_tokens(t_shell *shell, t_token *n_token, t_token *token, int j)
{
	...
	while (token->var_splited && token->var_splited[++k])
	{
		...
		if (no_quotes_outside(token->orig_input))//check for quotes outside of expanded var via orig_input
			n_token[j].expanded = true;
		else
			n_token[j].expanded = false;
