...
/*   Updated: 2024/03/20 11:08:49 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_file_perm(t_command *command, t_redirection *redirec, char *file_path)
{
	...
	if (!(type_redir(redirec) % 2) && !command->p_err_msg && access(file_path, R_OK) == -1)//IN
	{
	...
	if ((type_redir(redirec) % 2) && !command->p_err_msg && access(file_path, W_OK) == -1)//OUT
	{
		buffer = malloc(ft_strlen(redirec->file) + 50);
		if (!buffer)
			return (free(file_path), -1);
