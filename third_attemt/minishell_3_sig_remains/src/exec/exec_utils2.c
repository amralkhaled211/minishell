...
/*   Updated: 2024/03/19 19:14:57 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	output_redir_handler2(t_shell *shell, int **pfd, int i)
{
	...
	if (!shell->command[i].p_err_msg)
	{
		...
		my_sprintf(buffer, "minishell: %s: No such file or directory[IN CHILD]\n",
...
void	file_error_handling(t_shell *shell, int **pfd, int i)
{
	...
	if (!path_env && !shell->command[i].path)
	{
		...
		buffer = file_error_handline2(shell, i, path_env);
	write(2, buffer, ft_strlen(buffer));
	free(buffer);
