...
/*   Updated: 2024/02/25 13:37:36 by aismaili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_cmd_table(t_command *command)
{
	...
	while (!command[i].end_of_cmd)
	{
		...
		j = -1;
		printf("REDIR:\n");
		while (command[i].redirections && command[i].redirections[++j].file)
		{
...
void	print_tokens(t_shell *shell)
{
	...
	i = 0;
	while (shell->token[i].type != TOKEN_DEFAULT)
	{
...
void	print_n_tokens(t_shell *shell)
{
	...
	i = 0;
	while (shell->n_token[i].type != TOKEN_DEFAULT)
	{
		printf("i %d: Type = %d, Value = %s\n", i, shell->n_token[i].type, shell->n_token[i].value);
		i++;
	}
	printf("i %d: Type = %d, Value = %s\n", i, shell->n_token[i].type, shell->n_token[i].value);
}
