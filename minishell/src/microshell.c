...
int main(int ac, char *av[], char *envp[])
{
	...
	/* 	if (isatty(0))
		...
		// printf("	AFTER TOKEN ADV:\n");
		// print_cmd_table(shell.command);
		remove_quotes(&shell, shell.command);//protected
		handle_path(&shell, shell.command);//protected
		create_args(&shell, shell.command);//protected
		setup_redir(&shell, shell.command);//protected!!
		//printf("	AFTER REDIR SETUP:\n");
		//print_cmd_table(shell.command);
		begin_execution(&shell, shell.command);//kinda protected
...
int	execute_built_in(t_shell *shell, t_command *cmd)
{
	...
	return (res);//must be stored for echo $?
}
